...
select ol.ad_client_id, ol.ad_org_id, ol.m_product_id, ol.qtyordered - ol.qtydelivered as qtyremaining, ol.priceentered, 
...
(
	select p.name, c.m_product_id, p.ad_client_id, o.AD_Org_ID, c.M_AttributeSetInstance_ID, 
		...
		sch.c_currency_id, coalesce(pca.costinglevel, sch.costinglevel) as costinglevel, p.value
	from m_cost c
	...
	group by p.name, c.m_product_id, p.ad_client_id, o.AD_Org_ID, c.M_AttributeSetInstance_ID, 
		c.C_AcctSchema_ID, c.M_CostType_ID, sch.c_currency_id, coalesce(pca.costinglevel, sch.costinglevel), p.value
) tot
group by tot.name, tot.m_product_id, tot.ad_client_id, tot.AD_Org_ID, tot.M_AttributeSetInstance_ID, 
	tot.C_AcctSchema_ID, tot.M_CostType_ID, tot.c_currency_id, tot.costinglevel, tot.value
;