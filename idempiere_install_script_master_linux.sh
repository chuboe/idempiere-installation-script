...
if [[ $IS_INSTALL_ID == "N" && expr length $REPLICATION_URL -gt 0 ]]	
then
...
then
	...
	sudo sed -i "$ a\host    replication     id_replicate_role        0.0.0.0/0       md5" /etc/postgresql/$PGVERSION/main/pg_hba.conf
